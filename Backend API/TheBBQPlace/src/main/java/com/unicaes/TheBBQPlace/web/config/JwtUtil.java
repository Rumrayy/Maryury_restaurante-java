...
import com.unicaes.TheBBQPlace.persistence.entity.ReserveDetailsImpl;
...
public class JwtUtil {
    ...
    public UserDetails loadHistorybyUser(String email, Integer id) throws UsernameNotFoundException {
        ReserveEntity reserve = reserveService.getId(id);
        ClientEntity client = clientService.getprofile(email);

        if (reserve == null) {
            throw new UsernameNotFoundException("Historial no encontrado con email: " + email);
        }
        return new ReserveDetailsImpl(reserve,client);
    }
}
